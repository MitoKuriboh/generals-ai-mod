...
// FIX M1: Include TerrainLogic for map extent queries
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// ML DEBUG DISPLAY - Shows agent state on screen
///////////////////////////////////////////////////////////////////////////////////////////////////

// Global debug state for ML agent (accessible from debug display callback)
struct MLDebugState {
	Bool active;                    // Is Learning AI active?
	Bool connected;                 // Is ML bridge connected?
	Bool validRecommendation;       // Do we have valid recommendations?
	Real aggression;                // Current aggression level
	Real priorityEconomy;           // Economy priority
	Real priorityDefense;           // Defense priority
	Real priorityMilitary;          // Military priority
	Real priorityTech;              // Tech priority
	Real preferInfantry;            // Infantry preference
	Real preferVehicles;            // Vehicle preference
	Real preferAircraft;            // Aircraft preference
	Int teamsHeld;                  // Teams waiting to attack
	Int targetPlayer;               // Target player index
	UnsignedInt frameCounter;       // Current frame
	UnsignedInt lastDecisionFrame;  // Last ML decision frame
	char lastAction[64];            // Last action taken
	char connectionStatus[32];      // Connection status string
};

static MLDebugState g_mlDebugState = {0};

// Update the global debug state (called from AILearningPlayer::update)
static void updateMLDebugState(const MLRecommendation& rec, Bool connected, Int teamsHeld, UnsignedInt frame)
{
	g_mlDebugState.active = true;
	g_mlDebugState.connected = connected;
	g_mlDebugState.validRecommendation = rec.valid;
	g_mlDebugState.aggression = rec.aggression;
	g_mlDebugState.priorityEconomy = rec.priorityEconomy;
	g_mlDebugState.priorityDefense = rec.priorityDefense;
	g_mlDebugState.priorityMilitary = rec.priorityMilitary;
	g_mlDebugState.priorityTech = rec.priorityTech;
	g_mlDebugState.preferInfantry = rec.preferInfantry;
	g_mlDebugState.preferVehicles = rec.preferVehicles;
	g_mlDebugState.preferAircraft = rec.preferAircraft;
	g_mlDebugState.teamsHeld = teamsHeld;
	g_mlDebugState.targetPlayer = rec.targetPlayer;
	g_mlDebugState.frameCounter = frame;

	if (connected) {
		strcpy(g_mlDebugState.connectionStatus, "CONNECTED");
	} else {
		strcpy(g_mlDebugState.connectionStatus, "DISCONNECTED");
	}
}

static void setMLDebugAction(const char* action)
{
	strncpy(g_mlDebugState.lastAction, action, 63);
	g_mlDebugState.lastAction[63] = '\0';
	g_mlDebugState.lastDecisionFrame = g_mlDebugState.frameCounter;
}

// Debug display callback - renders ML state on screen
void MLAgentDebugDisplay(DebugDisplayInterface* dd, void* userData, FILE* fp)
{
	if (!g_mlDebugState.active) {
		dd->setCursorPos(2, 2);
		dd->setTextColor(DebugDisplayInterface::YELLOW);
		dd->printf("ML Agent: Not Active\n");
		return;
	}
	...
	// Header
	dd->setTextColor(DebugDisplayInterface::YELLOW);
	dd->printf("=== ML LEARNING AI ===\n");

	// Connection status
	if (g_mlDebugState.connected) {
		dd->setTextColor(DebugDisplayInterface::GREEN);
		dd->printf("Status: %s\n", g_mlDebugState.connectionStatus);
	} else {
		dd->setTextColor(DebugDisplayInterface::RED);
		dd->printf("Status: %s\n", g_mlDebugState.connectionStatus);
	}

	// Recommendation validity
	if (g_mlDebugState.validRecommendation) {
		dd->setTextColor(DebugDisplayInterface::GREEN);
		dd->printf("Recommendation: VALID\n");
	} else {
		dd->setTextColor(DebugDisplayInterface::RED);
		dd->printf("Recommendation: NONE (using fallback)\n");
	}
	...
	dd->printf("--- Build Priorities ---\n");
	dd->printf("Economy:  %3.0f%%\n", g_mlDebugState.priorityEconomy * 100.0f);
	dd->printf("Defense:  %3.0f%%\n", g_mlDebugState.priorityDefense * 100.0f);
	dd->printf("Military: %3.0f%%\n", g_mlDebugState.priorityMilitary * 100.0f);
	dd->printf("Tech:     %3.0f%%\n", g_mlDebugState.priorityTech * 100.0f);

	dd->printf("\n--- Unit Preferences ---\n");
	dd->printf("Infantry: %3.0f%%\n", g_mlDebugState.preferInfantry * 100.0f);
	dd->printf("Vehicles: %3.0f%%\n", g_mlDebugState.preferVehicles * 100.0f);
	dd->printf("Aircraft: %3.0f%%\n", g_mlDebugState.preferAircraft * 100.0f);

	dd->printf("\n--- Combat ---\n");

	// Aggression with color coding
	if (g_mlDebugState.aggression > 0.7f) {
		dd->setTextColor(DebugDisplayInterface::RED);
		dd->printf("Aggression: %3.0f%% [AGGRESSIVE]\n", g_mlDebugState.aggression * 100.0f);
	} else if (g_mlDebugState.aggression < 0.3f) {
		dd->setTextColor(DebugDisplayInterface::BLUE);
		dd->printf("Aggression: %3.0f%% [DEFENSIVE]\n", g_mlDebugState.aggression * 100.0f);
	} else {
		dd->setTextColor(DebugDisplayInterface::WHITE);
		dd->printf("Aggression: %3.0f%% [BALANCED]\n", g_mlDebugState.aggression * 100.0f);
	}

	dd->setTextColor(DebugDisplayInterface::WHITE);
	if (g_mlDebugState.teamsHeld > 0) {
		dd->setTextColor(DebugDisplayInterface::YELLOW);
		dd->printf("Teams Held: %d (waiting)\n", g_mlDebugState.teamsHeld);
	} else {
		dd->printf("Teams Held: 0\n");
	}

	if (g_mlDebugState.targetPlayer >= 0) {
		dd->printf("Target: Player %d\n", g_mlDebugState.targetPlayer);
	}

	// Last action
	dd->printf("\n--- Last Action ---\n");
	dd->setTextColor(DebugDisplayInterface::GREEN);
	dd->printf("%s\n", g_mlDebugState.lastAction);

	dd->setTextColor(DebugDisplayInterface::WHITE);
	dd->printf("\nFrame: %u\n", g_mlDebugState.frameCounter);
}
...
void AILearningPlayer::update()
{
	...
	if (m_frameCounter % MLConfig::DECISION_INTERVAL == 0) {
		...
		updateMLDebugState(m_currentRecommendation, m_mlBridge.isConnected(), m_teamsHeld, m_frameCounter);
	}
...
void AILearningPlayer::newMap()
{
	...
	// Enable ML debug display overlay
	if (TheDisplay) {
		TheDisplay->setDebugDisplayCallback(MLAgentDebugDisplay, NULL);
		setMLDebugAction("Initializing...");
	}
...
// M2 FIX: Single-pass force counting for both own and enemy forces
void AILearningPlayer::countAllForces(MLGameState& state)
{
	// Initialize arrays
	for (Int i = 0; i < 3; i++) {
		state.ownInfantry[i] = state.ownVehicles[i] = state.ownAircraft[i] = state.ownStructures[i] = 0.0f;
		state.enemyInfantry[i] = state.enemyVehicles[i] = state.enemyAircraft[i] = state.enemyStructures[i] = 0.0f;
	}

	// Counters for own forces
	Int ownInfCount = 0, ownVehCount = 0, ownAirCount = 0, ownStrCount = 0;
	Real ownInfHealth = 0.0f, ownVehHealth = 0.0f, ownAirHealth = 0.0f, ownStrHealth = 0.0f;

	// Counters for enemy forces
	Int enemyInfCount = 0, enemyVehCount = 0, enemyAirCount = 0, enemyStrCount = 0;
	Real enemyInfHealth = 0.0f, enemyVehHealth = 0.0f, enemyAirHealth = 0.0f, enemyStrHealth = 0.0f;
	...
	for (Object* obj = TheGameLogic->getFirstObject(); obj; obj = obj->getNextObject()) {
		...
		// Determine owner and categorize
		...
		// Skip neutral objects
		if (!isOwn && !isEnemy) continue;

		// Classify by type and accumulate
		if (obj->isKindOf(KINDOF_INFANTRY)) {
			if (isOwn) { ownInfCount++; ownInfHealth += healthRatio; }
			else { enemyInfCount++; enemyInfHealth += healthRatio; }
		} else if (obj->isKindOf(KINDOF_VEHICLE)) {
			if (isOwn) { ownVehCount++; ownVehHealth += healthRatio; }
			else { enemyVehCount++; enemyVehHealth += healthRatio; }
		} else if (obj->isKindOf(KINDOF_AIRCRAFT)) {
			if (isOwn) { ownAirCount++; ownAirHealth += healthRatio; }
			else { enemyAirCount++; enemyAirHealth += healthRatio; }
		} else if (obj->isKindOf(KINDOF_STRUCTURE)) {
			if (isOwn) { ownStrCount++; ownStrHealth += healthRatio; }
			else { enemyStrCount++; enemyStrHealth += healthRatio; }
		}
	}

	// Fill own forces: [0] = log10(count+1), [1] = avg health, [2] = production (stub)
	state.ownInfantry[0] = (Real)log10((double)(ownInfCount + 1));
	state.ownVehicles[0] = (Real)log10((double)(ownVehCount + 1));
	state.ownAircraft[0] = (Real)log10((double)(ownAirCount + 1));
	state.ownStructures[0] = (Real)log10((double)(ownStrCount + 1));

	state.ownInfantry[1] = (ownInfCount > 0) ? (ownInfHealth / ownInfCount) : 0.0f;
	state.ownVehicles[1] = (ownVehCount > 0) ? (ownVehHealth / ownVehCount) : 0.0f;
	state.ownAircraft[1] = (ownAirCount > 0) ? (ownAirHealth / ownAirCount) : 0.0f;
	state.ownStructures[1] = (ownStrCount > 0) ? (ownStrHealth / ownStrCount) : 0.0f;

	// Fill enemy forces
	state.enemyInfantry[0] = (Real)log10((double)(enemyInfCount + 1));
	state.enemyVehicles[0] = (Real)log10((double)(enemyVehCount + 1));
	state.enemyAircraft[0] = (Real)log10((double)(enemyAirCount + 1));
	state.enemyStructures[0] = (Real)log10((double)(enemyStrCount + 1));

	state.enemyInfantry[1] = (enemyInfCount > 0) ? (enemyInfHealth / enemyInfCount) : 0.0f;
	state.enemyVehicles[1] = (enemyVehCount > 0) ? (enemyVehHealth / enemyVehCount) : 0.0f;
	state.enemyAircraft[1] = (enemyAirCount > 0) ? (enemyAirHealth / enemyAirCount) : 0.0f;
	state.enemyStructures[1] = (enemyStrCount > 0) ? (enemyStrHealth / enemyStrCount) : 0.0f;
}

// Legacy function kept for compatibility (unused)
...
Bool AILearningPlayer::selectTeamToBuild()
{
	...
	if (teamProto) {
		...
		// Update debug display with selected team
		char actionBuf[64];
		snprintf(actionBuf, 64, "BUILD: %s", teamProto->getName().str());
		setMLDebugAction(actionBuf);
...
void AILearningPlayer::checkReadyTeams()
{
	...
	if (m_teamsHeld > 0) {
		...
		// Update debug display with attack action
		char actionBuf[64];
		snprintf(actionBuf, 64, "ATTACK: Release %d teams", m_teamsHeld);
		setMLDebugAction(actionBuf);
...
void AILearningPlayer::executeTacticalCommand(Team* team, const TacticalCommand& cmd)
{
	...
	// M1 FIX: Query actual map bounds instead of hardcoded scale
	Region3D mapExtent;
	TheTerrainLogic->getExtent(&mapExtent);
	Real mapWidth = mapExtent.hi.x - mapExtent.lo.x;
	Real mapHeight = mapExtent.hi.y - mapExtent.lo.y;
	// Use max dimension as scale (maps are typically square but may vary)
	Real mapScale = (mapWidth > mapHeight) ? mapWidth : mapHeight;
	if (mapScale < 100.0f) mapScale = 3000.0f;  // Fallback if extent not available

	Coord3D targetPos;
	targetPos.x = mapExtent.lo.x + cmd.targetX * mapWidth;
	targetPos.y = mapExtent.lo.y + cmd.targetY * mapHeight;
	targetPos.z = 0;
